...
#define NACIMIENTO_JULIANO 4
#define FILE_SIZE ( NOMBRES_SIZE + CEDULA_SIZE + CODIGO_SIZE + DIA_SIZE + MESANYO_SIZE + NACIMIENTO_JULIANO )
...
int main( int argc, char *argv[] )
{
   ...
   if ( argc != 2 ){
      printf( "%d*** ATENCION! Uso del comando:\n\tprograma archivo-de-datos\n",argc );
      return 1;
   }
   ...
   printf( "Diferencia anyo juliani:%d\n", datos.juliano);
...
/**
 * Lee del archivo un número binario de n bytes en representacion Big Endian en la variable apuntada por p.
 * El número se supone en formato big endian y lo convierte a little endian.
 */
unsigned int getNumberBE ( FILE * f, char * p, unsigned int n ){
...
/**
 * Lee del archivo un número binario de n bytes en representacion Little Endian en la variable apuntada por p.
 * El número se supone en formato big endian y lo convierte a little endian.
 */
...
unsigned int loadData ( struct Datos * datosP, char * nombreArchivo ){
   ...
   if ( !( f = fopen( nombreArchivo, "rb" ) ) ) {
      ...
      nBytes = NACIMIENTO_JULIANO;
      if ( getNumber( f, (char *)(&datosP->juliano), sizeof( datosP->juliano ) ) != nBytes ) {
